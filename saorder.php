...
if (isset($slug[2]) && !empty($slug[2])) :
	...
	if (isset($order['page_judul'])) :
		...
		if (isset($idmember) && is_numeric($idmember) && !isset($error)) {
			$lastidorder = db_var("SELECT `order_id` FROM `sa_order` ORDER BY `order_id` DESC LIMIT 0,1");
			if (!is_numeric($lastidorder)) {
				$lastidorder = 0;
			}

			$idunik = $lastidorder+1;
			if (strlen($idunik) > 3) {
				$idunik = substr($idunik, -3);
			}
			...
			if (isset($_POST['payment']) && !empty($_POST['payment'])) {
			    ...
			    $merchantRef  = str_pad($idunik,4,0,STR_PAD_LEFT);
				  ...
				  if (isset($settings['carapembayaran']) && !empty($settings['carapembayaran'])) { 
				    ...
				    <li class="list-group-item">
					    <div class="form-check">
						    ...
						    <strong>Pilih Tujuan Transfer</strong>
						    </label>
						  </div>
				    </li>'; 
				  }
