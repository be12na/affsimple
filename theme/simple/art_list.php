...
if (count($artikel) > 0) {
    ...
    <div class="col p-4 d-flex flex-column position-static">
      ...
      <div class="mb-1 text-body-secondary"><?= date('d M Y H:i',strtotime($artikel['art_tglpublish']));?> by <?= $artikel['mem_nama'];?></div>

      <p class="card-text mb-auto">	    
      <?= pendekin(strip_tags($artikel['art_konten']),170); ?></p>
      <a href="<?= $weburl.($settings['url_artikel']??='artikel').'/'.$artikel['art_slug'];?>" class="icon-link gap-1 icon-link-hover stretched-link">
        Selanjutnya..
	...
	<div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
		<div class="col p-4 d-flex flex-column position-static">
			<h2>Maaf, Belum ada Artikel 🙏</h2>
			<p>Sabar ya... kami sedang menyiapkan artikel yang oke buat kamu</p>
		</div>
	</div>';	
}
