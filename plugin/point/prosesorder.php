...
if (isset($proses['order_id'])) {
	...
	$ins = "(".$proses['order_id'].",".$proses['order_idmember'].",".$proses['order_idsponsor'].",'".date('Y-m-d H:i:s')."',".$proses['order_hargaunik'].",0,1,'".$keterangan."',0),";
			...
			if (count($upline) > 0) {				
				...
				foreach ($upline as $upline) {
					...
					if ($kredit > 0) {
						...
						$ins .= "(".$proses['order_id'].",".$proses['order_idmember'].",".$upline['mem_id'].",'".date('Y-m-d H:i:s')."',".$kredit.",0,2,'".cek($keterangan)."',".$lvl."),";
					}
					$lvl++;
				}			
			}
	...
	$cek = db_query("INSERT INTO `sa_laporan` (`lap_idorder`,`lap_idmember`,`lap_idsponsor`,`lap_tanggal`,`lap_masuk`,`lap_keluar`,`lap_code`,`lap_keterangan`,`lap_level`) 
	...
	if ($cek === false) {
		...
		if (isset($komisi['point'])) {
			$statussponsor = db_var("SELECT `mem_status` FROM `sa_member` WHERE `mem_id`=".$proses['order_idsponsor']);
			if (isset($komisi['point'][$statussponsor]) && $komisi['point'][$statussponsor] > 0) {
				$pointket = 'Point Penjualan '.$proses['page_judul'];
				$cek = db_insert("INSERT INTO `sa_point` (`poin_tanggal`,`poin_idmember`,`poin_idsponsor`,`poin_idorder`,`poin_jumlah`,`poin_keterangan`) 
					VALUES ('".date('Y-m-d H:i:s')."',".$proses['order_idmember'].",".$proses['order_idsponsor'].",".$proses['order_id'].",".$komisi['point'][$statussponsor].",'".cek($pointket)."')");
				if ($cek === false) {
					echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
					  <strong>Error!</strong> '.db_error().'
					  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
				...
				echo 'Tidak ada point untuk status: '.$statussponsor;
			}
		}
