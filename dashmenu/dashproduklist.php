	...
	<div class="card-body">
	  <div class="row">	    
	    ...
	    <div class="col-sm-2 text-end">	    	
	    	<a href="?edit=add" class="btn btn-success">Tambah Produk</a>
	    </div>
	  </div>
	</div>
</div>
	...
	<tbody class="table-group-divider">
		...
		if (count($data) > 0) {			
			...
			if ($data['pro_status'] == 1) {
				echo '
					<a href="produk?nonaktif='.$data['page_id'].'" class="btn btn-warning mr-3" title="Nonaktif produk"><i class="fa-regular fa-circle-stop"></i> Nonaktif</a>
					&nbsp;';
			} else {
				echo '
					<a href="produk?aktif='.$data['page_id'].'" class="btn btn-primary mr-3" title="Aktifkan produk"><i class="fa-regular fa-circle-play"></i> Aktifkan</a>
					&nbsp;';
			}
...
<?php
$jmlmember = db_var("SELECT * FROM `sa_page` 
			WHERE `pro_harga` IS NOT NULL ".$where);
