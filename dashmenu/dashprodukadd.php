...
if (isset($_POST['urlpage']) && !empty($_POST['urlpage']) && isset($_POST['judulpage']) && !empty($_POST['judulpage'])) {
    ...
    $pic_dir = str_replace('dashmenu/dashprodukadd.php','upload',__FILE__);
	...
	if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
		...
		# Simpan di database
		$cek = db_query("INSERT INTO `sa_page` (`page_judul`,`page_diskripsi`,`page_url`,`page_iframe`,`pro_harga`,`pro_komisi`,`pro_file`,`pro_img`) VALUES 
			('".cek($_POST['judulpage'])."','".cek($_POST['diskripsipage'])."','".cekurlpage($_POST['urlpage'])."','".cek($_POST['iframe'])."',".numonly($_POST['harga']).",'".$dbkomisi."','".cek($_POST['namafile'])."','".$gambar."')");
	}
