...
$head['pagetitle'] ='Jaringan Member';
showheader($head);
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
	$data = db_select("SELECT * FROM `sa_sponsor` LEFT JOIN `sa_member` ON `sa_member`.`mem_id`=`sa_sponsor`.`sp_mem_id` WHERE `sp_sponsor_id`=".$_GET['id']);
	if (count($data) > 0) {
		#$status = array('Blm Valid','Free Member','Premium','Qualified');
		...
		$j(function(){
			$j(document).on(\'click\',\'.folder\', function() {
				...
				if ( $j("#downline"+idmember ).length ) {
					...
					$j.get("'.$weburl.'/member", { id: this.id },
					   	function(data){
					    	$j("#load").remove();
			...
			$j(document).on(\'click\',\'.detil\', function() {
				...
				$j.get("'.$weburl.'/member", { member: this.id },
				   	function(data){
				    	$j("#load").remove();
